...
const BbmInput = () => {
  ...
  const handleSubmit = async () => {
    ...
    const kmAwalRaw = String(formData['kilometer-awal'] || '').trim();
    const kmAkhirRaw = String(formData['kilometer-akhir'] || '').trim();
    const kmAwal = kmAwalRaw ? Number(kmAwalRaw) : NaN;
    const kmAkhir = kmAkhirRaw ? Number(kmAkhirRaw) : NaN;
    if (kmAwalRaw && Number.isNaN(kmAwal)) nextErrors['kilometer-awal'] = 'Harus angka';
    if (kmAkhirRaw && Number.isNaN(kmAkhir)) nextErrors['kilometer-akhir'] = 'Harus angka';
    if (!Number.isNaN(kmAwal) && !Number.isNaN(kmAkhir) && kmAkhir < kmAwal) {
      nextErrors['kilometer-akhir'] = 'Kilometer akhir tidak boleh lebih kecil dari kilometer awal';
    }
    ...
    const payload = {
      ...
      kilometer_awal: kmAwalRaw,
      kilometer_akhir: kmAkhirRaw,
      messenger: formData['messenger'],
    };
    const res = await createBBMRecord(payload);
    const bbmId = res?.data?.id ?? res?.data?.data?.id ?? null;
    if (res?.success && bbmId) {
      for (const f of uploadedFiles) {
        try {
          await fileAPI.upload(f.file, 'bbm_proof', { bbm_record_id: bbmId });
        ...
        <div className="w-full max-w-4xl mx-auto bg-white rounded-[17.38px] shadow-[0px_0px_0.91px_#0000000a,0px_1.83px_5.49px_#0000000a,0px_14.63px_21.95px_#0000000f] translate-y-[-1rem] animate-fade-in opacity-0 [--animation-delay:0ms]">
          ...
          <section className="flex flex-col w-full items-end gap-4 sm:gap-6 px-4 sm:px-6 lg:px-9 pb-6 lg:pb-8 translate-y-[-1rem] animate-fade-in opacity-0 [--animation-delay:200ms]">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto sm:justify-end mt-4">
              <Link to="/bbm" className="w-full sm:w-auto">
